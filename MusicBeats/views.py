...
def watchlater(request):
    ...
    w1 = Watchlater.objects.filter(user = request.user)
    ids = []
    for i in w1:
...
def upload(request):
    if request.method == "POST":
        ...
        song_model = Song(name = name,singer = singer,genre=genre,image = image,movie = movie,credit = credit, song = song1)
        song_model.save()
    ...
    if request.method == "POST":
        ...
        for i in fav:
            if fav_song_id == i.fav_song_id:
                message = "Your song is already in Favourites"
                break
        else:                   
